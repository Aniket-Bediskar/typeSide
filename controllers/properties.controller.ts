...
export const getCountsForManagerAllTable = async (req: Request, res: Response) => {
  ...
  try {
      ...
      include: [
        {
          ...
          include: {
            model: models.units,
            attributes: ['id','units_name', 'isAssign'] as any,
          },
        },
      ] as any,
    ...
    const formattedCounts = properties.map((property) => {
      const { id:propertyId,property_name, blocks } = property;
      const formattedBlocks = blocks.map((block:any) => {
        const { id: blockId,blocks_name:blocks_name, units } = block;
        const formattedUnits = units.map((unit:any) => ({
          id: unit.id,
          isAssign: unit.isAssign,
          units_name:unit.units_name
        }));
        return { blockId, blocks_name,units: formattedUnits };
      });
      ...
      return {propertyId, property_name, blocks: formattedBlocks, occupiedUnits, vacantUnits };
    });
