...
export const getTicketWithChat = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (ticket) {
      const createdByUserId = ticket.ticketCreatedBy;
...
export const getAllTicketsForManager = async (req: Request, res: Response) => {
  const models: Models = req.app.locals.models;
  const { propertyId } = req.params;
  ...
  try {
    ...
    const ticketsArray = propertyData.flatMap((property) => property.tickets);

    return res.status(200).json({ status: 200, data: ticketsArray });
  } catch (error) {
    console.error("Error in fetching property tickets", error);
    return res
      .status(500)
...
export const findTicketById = async (
  ...
  models: {
    ticket: {
      findByPk: (
        arg0: any,
        arg1: { include: { model: any; required: boolean }[] }
      ) => any;
    };
    chatModel: {
      update: (
        arg0: { status: string },
        arg1: { where: { ticketId: any; status: string } }
      ) => any;
    };
  },
